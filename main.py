...
class FunPaySteamBot:
    ...
    async def handle_message(self, suffix: str, data: Dict):
        ...
        try:
            ...
            message_id = str(data.get('id') or data.get('messageId') or data.get('msg_id'))

            if not all([chat_id, message_id]):
                logger.warning(f"{suffix}: Неполные данные: {data}")
                return
            
            if await self.db.is_message_processed(message_id):
            ...
            if response:
                ...
                if client:
                    ...
                    await self.db.mark_message_processed(message_id, suffix)
            ...
            await self.db.log_event(suffix, "error", str(e))
